function [magD1, magD2, mag3, magD4, phaseD11, phaseD21, phaseD31, phaseD41, t1] = getMeas()
clear; clear all;
% +++++++++++++++++++++++++++++++++++++++++++ Reader 1 ++++++++++++++++++++++++++++++++++++++
...
%
mag3 = NaN(length(t1),1);
mag3(1) = magD1(1);   

phase3 = NaN(length(t1),1);
phase3(1) = phaseD3(1);   
...
for indexMag = 1:1:length(time3)
    if indexT  <= length(time1)&& (abs(time3(indexMag) - time1(indexT)) < 0.0002 || ((time3(indexMag)) > time1(indexT)))
        ...
        phase3(indexT) = phaseD3(indexMag);
        
        indexT = indexT + 1;
    end
end

phaseDiff1 = diff(phaseD1); phaseDiff2 = diff(phaseD2);  phaseDiff3 = diff(phase3);  phaseDiff4 = diff(phaseD4); 

phaseDiff1 = -medfilt1(phaseDiff1,4000)*1/(42e-03);
phaseDiff2 = -medfilt1(phaseDiff2,4000)*1/(42e-03);
phaseDiff3 = medfilt1(phaseDiff3,4000)*1/(42e-03);
phaseDiff4 = -medfilt1(phaseDiff4,4000)*1/(42e-03);

phaseD11 = NaN(1,length(phaseDiff1));
phaseD21 = NaN(1,length(phaseDiff1));
phaseD31 = NaN(1,length(phaseDiff1));
phaseD41 = NaN(1,length(phaseDiff1));
%%
phaseD11(1:end-1300) = phaseDiff1(1301:end);
phaseD21(1:end-1300) = phaseDiff2(1301:end);
phaseD31(1:end-1300) = phaseDiff3(1301:end);
phaseD41(1:end-1300-8) = phaseDiff4(1301:end);

phaseD11(end-1300+1:end) = phaseDiff1(1:1300);
phaseD21(end-1300+1:end) = phaseDiff2(1:1300);
phaseD31(end-1300+1:end) = phaseDiff3(1:1300);
phaseD41(end-1300-7:end) = phaseDiff4(1:1308);
end
