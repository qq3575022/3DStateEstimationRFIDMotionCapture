...
x = [0.5;0.5; 0.5;0.5; 1.1;0.5]*ones(1,length(rtime)-N);e = NaN(1,length(rtime)-N);
factor = 1;
...
for m = 1:1:length(rtime)-N
    ...
    x(:,m) = lsqnonlin(@(xx)getNLE(y, xx, N, index, len, rtime, m, factor),[0.5;0.5; 0.5;0.5; 0.5;0.5;]);
...
subplot(9,1,4), plot(rtime(1:end-N), x(3,:), 'b', 'LineWidth', 2); hold on; plot(rtime, coord3(2,1:end), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated orientation along x axis'); grid on;
subplot(9,1,5), plot(rtime(1:end-N), x(4,:), 'b', 'LineWidth', 2); hold on; plot(rtime, coord3(5,1:end), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated angular velocity along x axis'); grid on;
subplot(9,1,7), plot(rtime(1:end-N), x(5,:), 'b', 'LineWidth', 2); hold on; plot(rtime, coord3(3,1:end), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated angular velocity along y axis'); grid on;
subplot(9,1,8), plot(rtime(1:end-N), x(6,:), 'b', 'LineWidth', 2); hold on; plot(rtime, coord3(6,1:end), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated orientation along z axis'); grid on;
