...
yyS = find(abs(gyro_time-57.9176)<0.002);  yyS = yyS(1);
yyE = find(abs(gyro_time-61.0160)<0.002); yyE = yyE(1);

ySS = find(abs(mag_time-57.9176)<0.012);   ySS = ySS(1);
...
zzS = find(abs(gyro_time-67.9376)<0.002); zzS = zzS(1);
zzE = find(abs(gyro_time-70.0537)<0.002); zzE = zzE(1);

zSS = find(abs(mag_time-67.9376)<0.009); zSS = zSS(1);
...
%
[SimPP1, SimVV1, SimAA1] = groundtruth1Dy(accT-accT(1));
...
time1 = t1(70079:84485) +13.5167; time3 = t3(70079*2:84485*2)+13.5167;
mag11 = mag1(70079:84485); mag22 = mag2(70079:84485); mag33 = mag3(70079*2:84485*2); mag44 = mag4(70079:84485); 

%
figure
subplot(411), plot(mag1)
subplot(412), plot(mag2)
subplot(413), plot(mag3)
subplot(414), plot(mag4)
%%
phase11 = phase1(70079:84485); phase22 = phase2(70079:84485); phase33 = phase3(70079*2:84485*2); phase44 = phase4(70079:84485); 
...
measr1 = 0.016*mag11.^(-0.5)-0.5; measr2 = 0.006*mag22.^(-0.5)+1.22; 
measr3 = 0.007*mag33.^(-0.5)+1.4; measr4 = 0.01*mag44.^(-0.5)+0.6;
...
figure
subplot(411), plot(time1,  measr1, 'LineWidth', 1), hold on, plot(time, r_sim1, 'LineWidth', 1), title('Radial Distance $r1$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(412), plot(time1,  measr2, 'LineWidth', 1), hold on, plot(time, r_sim2, 'LineWidth', 1),title('Radial Distance $r2$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(413), plot(time3,  measr3, 'LineWidth', 1), hold on, plot(time, r_sim3, 'LineWidth', 1),title('Radial Distance $r3$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(414), plot(time1,  measr4, 'LineWidth', 1), hold on, plot(time, r_sim4, 'LineWidth', 1),title('Radial Distance $r4$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);



figure
subplot(411), plot(time1(2:end), phaseDiff1); hold on; plot(time, rdot_sim1, 'LineWidth', 1), title('Radial Velocity $\dot r_1$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(412), plot(time1(2:end), phaseDiff2); hold on; plot(time, rdot_sim2, 'LineWidth', 1), title('Radial Velocity $\dot r_2$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(413), plot(time3(2:end), phaseDiff3); hold on; plot(time, rdot_sim3, 'LineWidth', 1), title('Radial Velocity $\dot r_3$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
subplot(414), plot(time1(2:end), phaseDiff4); hold on; plot(time, rdot_sim4, 'LineWidth', 1), title('Radial Velocity $\dot r_4$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([57.9531, 61.4569]);
...
phase11 = phase1(70079:84485); 
time1 = t1(70079:84485);

phaseDiff = diff(phase11);
phaseDiff = medfilt1(phaseDiff,1000);

figure
subplot(311), plot(time, x(1,:), 'LineWidth', 2); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([57.9408, 61.456])
subplot(312), plot(time, x(2,:), 'LineWidth', 2); hold on; plot(time, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([57.9408, 61.456])%xlim([3.5, 7.5])
subplot(313), plot(time, x(3,:), 'LineWidth', 2); hold on; plot(time, AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along x Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([57.9408, 61.456])
